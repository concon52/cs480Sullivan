...
int Rotation = 1;
float Speed = 0;
char* PlanetStatus = (char*)"Model: Counterclockwise";
...
Vertex* LoadModelAssImp(char* FilePath);
...
int main(int argc, char **argv)
{
    ...
    glutAddMenuEntry("Reverse Model Rotation", 2);
...
void render()
{
    ...
    Text = (char*) "A.................Reverse Model Rotation";
    Text2 = (char*)"Left arrow.....CounterClockwise Model Rotation";
    Text3 = (char*)"Right arrow...Clockwise Model Rotation";

    Sprint(-0.8,0.9,Text);
    Sprint(-0.8,0.8,Text2);
    Sprint(-0.8,0.7,Text3);
...
void update()
{
    ...
    rotate += dt * 0.8 * (Rotation);
    model = glm::translate( glm::mat4(1.0f), glm::vec3(0.0, -2.0, 0.0));
...
void ArrowKeys(int button, int x_pos, int y_pos)
{
    ...
    {
        Rotation = 1;
        PlanetStatus = (char*)"Model: Counterclockwise";
    }

    if (button == GLUT_KEY_RIGHT)
    {
        Rotation = -1;
        PlanetStatus = (char*)"Model: Clockwise";
    }
}
...
bool initialize(char* arg)
{
    ...
    Vertex* geometry = LoadModelAssImp(arg);
...
Vertex* LoadModelAssImp(char* FilePath)
{
    ...
    for (unsigned int i=0; i<(mesh->mNumFaces); i++)
    {
        for (int j=0; j<3; j++)
        {
            (Geometry)->position[0] = mesh->mVertices[(mesh->mFaces[i]).mIndices[j]].x;
            (Geometry)->position[1] = mesh->mVertices[(mesh->mFaces[i]).mIndices[j]].y;
            (Geometry)->position[2] = mesh->mVertices[(mesh->mFaces[i]).mIndices[j]].z;

                (Geometry)->color[0] = ((float)rand())/ColorVariable;
                (Geometry)->color[1] = ((float)rand())/ColorVariable;
                (Geometry)->color[2] = ((float)rand())/ColorVariable;
